...
RC WriteFile(PF_Manager &pfm, char *fname) {
  ...
  for (i = 0; i < PF_BUFFER_SIZE; i++) {
    if ((rc = fh.AllocatePage(ph)) || (rc = ph.GetData(pData))
...
RC ReadFile(PF_Manager &pfm, char* fname) {
  ...
  if ((rc = pfm.OpenFile(fname, fh)) || (rc = PrintFile(fh))
      || (rc = pfm.CloseFile(fh)))
    return (rc);
  else
    return (0);
...
RC TestPF() {
  ...
  if ((rc = pfm.CreateFile(FILE1)) || (rc = pfm.CreateFile(FILE2)) || (rc =
      WriteFile(pfm, (char*) FILE1)) || (rc = ReadFile(pfm, (char*) FILE1))
      || (rc = WriteFile(pfm, (char*) FILE2))
      || (rc = ReadFile(pfm, (char*) FILE2)) || (rc = pfm.OpenFile(FILE1, fh1))
  ...
  if ((rc = fh1.FlushPages()) || (rc = fh2.FlushPages())
      || (rc = pfm.CloseFile(fh1)) || (rc = pfm.CloseFile(fh2)) || (rc =
          ReadFile(pfm, (char*) FILE1)) || (rc = ReadFile(pfm, (char*) FILE2))
      || (rc = pfm.DestroyFile(FILE1)) || (rc = pfm.DestroyFile(FILE2)))
    return (rc);

  cout << "Creating and opening files again\n";

  if ((rc = pfm.CreateFile(FILE1)) || (rc = pfm.CreateFile(FILE2)) || (rc =
      WriteFile(pfm, (char*) FILE1)) || (rc = WriteFile(pfm, (char*) FILE2))
      || (rc = pfm.OpenFile(FILE1, fh1)) || (rc = pfm.OpenFile(FILE2, fh2)))
    return (rc);

  cout << "Allocating additional pages in both files\n";

  for (i = PF_BUFFER_SIZE; i < PF_BUFFER_SIZE * 2; i++) {
    if ((rc = fh2.AllocatePage(ph)) || (rc = ph.GetData(pData))
        || (rc = ph.GetPageNum(pageNum)))
      return (rc);
    ...
    if ((rc = fh2.MarkDirty(pageNum)) || (rc = fh2.UnpinPage(pageNum)))
      return (rc);

    if ((rc = fh1.AllocatePage(ph)) || (rc = ph.GetData(pData))
        || (rc = ph.GetPageNum(pageNum)))
      return (rc);
    ...
    if ((rc = fh1.MarkDirty(pageNum)) || (rc = fh1.UnpinPage(pageNum)))
      return (rc);
  }
  ...
  for (i = PF_BUFFER_SIZE; i < PF_BUFFER_SIZE * 2; i++) {
    if (i & 1) {
      if ((rc = fh2.GetThisPage(i, ph)) || (rc = ph.GetData(pData))
          || (rc = ph.GetPageNum(pageNum)))
        return (rc);
  ...
  for (i = PF_BUFFER_SIZE; i < PF_BUFFER_SIZE * 2; i++) {
    if (!(i & 1)) {
      if ((rc = fh1.GetThisPage(i, ph)) || (rc = ph.GetData(pData))
          || (rc = ph.GetPageNum(pageNum)))
        return (rc);
  ...
  if ((rc = PrintFile(fh2)) || (rc = PrintFile(fh1)))
    return (rc);
  ...
  if ((rc = PrintFile(fh1)) || (rc = pfm.CloseFile(fh1))
  ...
  if (temp != 1 || pageNum != 1) {
    cout << "Asked for page 1, got: " << (int) pageNum << " " << (int) temp
         << "\n";
    exit(1);
  }
  ...
  if ((rc = PrintFile(fh1)) || (rc = PrintFile(fh2)))
    return (rc);

  cout << "Closing and destroying both files\n";

  if ((rc = pfm.CloseFile(fh1)) || (rc = pfm.CloseFile(fh2))
      || (rc = pfm.DestroyFile(FILE1)) || (rc = pfm.DestroyFile(FILE2)))
    return (rc);
